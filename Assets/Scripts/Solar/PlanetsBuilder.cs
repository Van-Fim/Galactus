...
{
    public static void Build(SpaceSystem starSystem)
    {
        LoadSuns(starSystem);
    ...
    public static void LoadSuns(SpaceSystem starSystem)
    {
        ...
        if (sunNodes.Count > 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Sun sun = new Sun(starSystem, templateSName, minRange, maxRange);
    ...
    public static void LoadPlanets(SpaceSystem starSystem)
    {
        ...
        if (planetNodes.Count > 0)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                Planet planet = new Planet(starSystem, sun, templateSName, minRange, maxRange);
                planet.Init();
            }
        }
    }
