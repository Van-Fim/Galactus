...
public class Planet : SolarObject
{
    ...
    public Planet() { }
    public Planet(SpaceManager spm, StarSystem system, Sun sun, string templateName, int minRange = 0, int maxRange = 0)
    {
        spaceManager = spm;
    ...
    public Planet(SpaceManager spm, Planet planet, string templateName, int minRange = 0, int maxRange = 0)
    {
        this.parentSolarObject = planet;
        this.spaceManager = spm;
    ...
    public override void AddSattelites()
    {
        ...
        if (satellitesNodes.Count > 0)
        {
            for (int i = 0; i < satelliteCount; i++)
            {
                ...
                int satelliteMaxRange = int.Parse(satelliteNode.GetValue("maxRange"));
                Planet satellitePlanet = new Planet(this.spaceManager, this, satelliteTemplateName, satelliteMinRange, satelliteMaxRange);
    ...
    public override void RenderAct()
    {
        if ((!(spaceManager is MapSpaceManager) && LocalClient.GetGalaxyId() == galaxyId && LocalClient.GetSystemId() == systemId) || (spaceManager is MapSpaceManager && MapSpaceManager.selectedGalaxyId == galaxyId && MapSpaceManager.selectedSystemId == systemId && MapClientPanel.currentLayer > 1))
        {
            if (main == null)
            {
                ...
                {
                    //solarController.transform.localPosition = GetPosition() * SolarObject.scaleFactor / Sector.minimapDivFactor;
                    //fscale = scale  * SolarObject.scaleFactor / Sector.minimapDivFactor;
